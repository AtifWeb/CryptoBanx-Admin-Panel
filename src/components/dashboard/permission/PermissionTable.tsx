...
export const PermissionTable = ({ fees, setsetfees, not_found }: any) => {
        ...
        {fees.length == 0 && (
          <div className="flex justify-center pt-8">
            {not_found == true ? (
              ...
              <h1 className="text-[20px] font-bold">Fetching Fee...</h1>
            )}
          </div>
        )}
      </div>

      <div className="flex items-center justify-end gap-2 mt-5 col-span-2">
        <Button
          onClick={() => setaddfee(true)}
          variant="outline"
          className="w-[135px]  cursor-pointer h-[42px] rounded-[8px] text-[16px] text-[#525A65] "
        >
          <svg
            width="21"
            height="21"
        ...
        <Button
          className="w-[117px] cursor-pointer h-[42px] rounded-[8px] text-[16px] text-[#FFFFFF] bg-gradient-to-r  from-[#194EAD] to-[#3793D2]
            "
        >
          Save
        </Button>
