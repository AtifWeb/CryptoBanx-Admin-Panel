import { affiliate_user } from "@/api/affiliate";
import { Switch } from "@mui/joy";
import React, { useEffect, useState } from "react";
import Modal from "@mui/joy/Modal";
import { Button } from "@/components/ui/button";
import { ModalDialog } from "@mui/joy";
export const SwitchUserAffiliate = ({ validated }: any) => {
  const [ischecked, setischecked] = useState(false);
  const [active, setactive] = useState(false);
  const [open, setopen] = useState(false);
  useEffect(() => {
    setischecked(validated.affiliate_active);
    console.log(validated["Affiliate Active"]);
    if (validated["Affiliate Active"] != undefined) {
      setischecked(validated["Affiliate Active"]);
    }
  }, []);
          ...
          <div className="px-[50px]">
            ...
            <p className="text-[16px] mb-4 mx-auto mt-3 text-[#454850] font-normal text-center">
              ...
              <span className="text-[#050A18] font-semibold">
                {ischecked == false ? "Affiliate" : "Remove From Affiliate"}
              </span>{" "}
              this account
            </p>
              ...
              <Button
                onClick={() => {
                  if (validated.Id != undefined) {
                    affiliate_user(
                      {
                        change_user_affiliate_state_user_id: validated.Id,
                        change_user_affiliate_state_activate: !ischecked,
                        change_user_affiliate_state_payment: 10,
                      },
