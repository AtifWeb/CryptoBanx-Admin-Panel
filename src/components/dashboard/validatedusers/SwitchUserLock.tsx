import { lock_state_user } from "@/api/admin";
import { Switch } from "@mui/joy";
import React, { useEffect, useState } from "react";
import Modal from "@mui/joy/Modal";
import { Button } from "@/components/ui/button";
import { ModalDialog } from "@mui/joy";
export const SwitchUserLock = ({ validated }: any) => {
  const [ischecked, setischecked] = useState(false);
  const [active, setactive] = useState(false);
  const [open, setopen] = useState(false);
  useEffect(() => {
    setischecked(validated.locked);

    if (validated.Locked != undefined) {
      setischecked(validated.Locked);
    }
  }, []);
              ...
              <Button
                onClick={() => {
                  if (validated.Id != undefined) {
                    lock_state_user(
                      {
                        change_user_lock_state_user_id: validated.Id,
                        change_user_lock_state_locked: !ischecked,
                        change_user_lock_state_user_type:
                          validated["Is Admin"] == true ? "admin" : "user",
                        change_user_lock_state_lock_count: 3,
                      },
